...
layui.define(['laypage', 'layer', 'form', 'pagesize', 'common','laydate'], function (exports) {
    ...
    function initilData(currentIndex, pageSize) {
        ...
        setTimeout(function () {
            ...
            html += '<thead><tr><th>编号id</th><th>学生</th><th>每日计划</th><th>老师建议</th><th >辅导员审核</th><th >是否完成</th><th >创建时间</th><th >修改时间</th><th  colspan="3">操作</th></tr></thead>';
            html += '<tbody>';
            //遍历文章集合
            for (var i = 0; i < data.length; i++) {
                ...
                html += '<td><button class="layui-btn layui-btn-small layui-btn-normal" onclick="layui.adminDailyPlanning.editData(\'' + item.id + ',' + item.plan + '\')"><i class="layui-icon">&#xe60a;</i></button></td>';
                html += '<td><button class="layui-btn layui-btn-small layui-btn-warm" onclick="layui.adminDailyPlanning.auditData(\'' + item.id + '\')"><i class="layui-icon">&#xe60a;</i></button></td>';
                html += '<td><button class="layui-btn layui-btn-small layui-btn-danger" onclick="layui.adminDailyPlanning.deleteData(\'' + item.id + '\')"><i class="layui-icon">&#xe640;</i></button></td>';
                html += "</tr>";
            }
            html += '</tbody>';
            html += '</table>';
    ...
    form.on('submit(getEditUpadat)', function (data) {
        ...
        common.ajax(common.onlineIp+'/arrangements/update', 'post', 'json', {
            'id': data.field.id,//学生
    ...
    function getEditUpadat(id, plan) {
        ...
        var strHtml = '        <form lay-filter="formTest" class="layui-form" style="padding-top: 10px">\n' +
            ...
            '                <div class="layui-input-block " style="padding-right: 20px;">\n' +
            '                    <textarea name="suggest" placeholder="请输入内容" lay-verify="required"  class="layui-textarea">' + suggest + '</textarea>\n' +
            '                </div>\n' +
            '            </div>\n' +
    ...
    exports('adminDailyPlanning', adminDailyPlanning);
});
