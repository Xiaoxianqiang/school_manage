...
layui.define(['element', 'layer', 'util', 'pagesize', 'form','common'], function (exports) {
    var $ = layui.jquery;
    ...
    //获取ip  与城市 与时间
    common.ajax(common.onlineIp+'/getIp', 'post', 'json', {
    }, function (res) {
        $(".ip").html(res);
    })
    ...
    common.ajax(common.onlineIp+'/user/list', 'post', 'json', {
        ...
        $("#studentUsers").html(res.data.list.length )
    ...
    common.ajax(common.onlineIp+'/user/list', 'post', 'json', {
        ...
        $("#counselorUser").html(res.data.list.length )
    })
    //每日规划
    common.ajax(common.onlineIp+'/arrangements/list', 'post', 'json', {
        ...
        $("#studentDailyPlanning").html(res.data.list.length)
    })
    //启航
    common.ajax(common.onlineIp+'/questionnaire/list', 'post', 'json', {
        ...
        $("#theHostelHasSetSailList").html( res.data.list.length)
    })
    ...
    $("#theCancellation").click(function(){
        layui.data('data', null);
        layer.msg('注销成功，正在跳转......', { icon: 6 });
        layer.closeAll('page');
        setTimeout(function () {
            location.href = "../html/indexAdmin.html";
        }, 1000);
    })
